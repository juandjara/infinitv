...
import { LockClosedIcon as LockIcon } from '@heroicons/react/solid'
...
export default function SignUp() {
  ...
  async function handleSubmit(ev) {
    ...
    setLoading(true)
    const { error } = await supabase.auth.signUp({ email, password }, { redirectTo: window.location.origin })
    if (error) {
      console.error(error)
  ...
  return (
    <main className="flex flex-col items-center justify-center flex-auto my-4 px-3">
      <div style={{ maxHeight: 640 }} className="flex md:h-full justify-between bg-white text-gray-700 rounded-lg">
